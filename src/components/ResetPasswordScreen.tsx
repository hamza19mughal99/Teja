...
import { resetPasswordUser, clearError, clearSuccessMessage } from '../store/slices/AuthSlice';
...
export default function ResetPasswordScreen({ onNavigate, onLogin }: ResetPasswordScreenProps) {
    ...
    const handleReset = async (e: React.FormEvent) => {
        ...
        if (password !== passwordConfirmation) {
            // Just using the UI state to handle mismatch isn't strictly necessary since we can pass it natively, 
            // but we'll let API handle validation to keep it simple, or we can check here.
            alert("Passwords do not match");
            return;
        }

        const resultAction = await dispatch(resetPasswordUser({ code, password, passwordConfirmation }));

        if (resetPasswordUser.fulfilled.match(resultAction)) {
            onLogin(); // Go straight to dashboard on success, since API logs you in
        }
    };
        ...
        <div className="min-h-screen w-full flex items-center justify-center bg-white p-6">
            <div className="relative w-full max-w-md">
                <div className="text-center mb-10">
                    ...
                    <p className="text-gray-600">Enter your reset code and choose a new password</p>
                </div>

                <div className="bg-white rounded-3xl shadow-lg border border-gray-200 p-8 lg:p-10">
                    <form onSubmit={handleReset} className="space-y-6">
                        <div className="space-y-2">
                            <label htmlFor="code" className="text-sm font-semibold text-gray-700 block">
                                Reset Code
                            </label>
                            <input
                                id="code"
                        ...
                        <div className="space-y-2">
                            <label htmlFor="passwordConfirmation" className="text-sm font-semibold text-gray-700 block">
                                Confirm Password
                            </label>
                            <div className="relative">
                                <input
                        ...
                        {error && (
                            <div className="text-sm text-red-600 font-medium p-2 bg-red-50 rounded-lg border border-red-100">
                                {typeof error === 'string' ? error : 'Password reset failed. Please check your code.'}
                            </div>
                        )}

                        {successMessage && (
                            <div className="text-sm text-green-700 font-medium p-2 bg-green-50 rounded-lg border border-green-200">
                                {successMessage}
                            </div>
                        )}
